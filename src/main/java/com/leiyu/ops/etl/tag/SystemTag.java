...
public class SystemTag {
	private static final Logger logger = LoggerFactory.getLogger(SystemTag.class);
	...
	public static void init(String batchNo) {
		...
		// system_tag table
		String queryByCrowdId = "select * from persona.system_tag where crowd_id = " + crowdId;
		params.clear();
		Map<String, Object> system_tag = DbUtil.queryOne(queryByCrowdId, params);
		String tagId = system_tag.get("id").toString();
		logger.info("=========tagId========" + tagId);
		String scriptParam = system_tag.get("script_param").toString();
		Map<String, String> param = JSON.parseObject(scriptParam, Map.class);
		if (param.containsKey("count")) {
			String count = param.get("count");
			contentMap.put("count", count);
		} else {
			String tableName = param.get("tableName");
			String keyColumn = param.get("keyColumn");
			String valueColumn = param.get("valueColumn");
			String range = param.get("range");
			contentMap.put("tableName", tableName);
			contentMap.put("keyColumn", keyColumn);
			contentMap.put("valueColumn", valueColumn);
			contentMap.put("dataRange", range);
		}
		logger.info("=========scriptParam========" + param);
		// fixColumns
